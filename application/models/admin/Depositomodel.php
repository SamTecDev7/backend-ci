...
class Depositomodel extends CI_Model{
    ...
    public function NovaConta(){
        ...
        if($insert){
            ...
            return '<div class="alert alert-success text-center">Conta para pagamento adicionada com sucesso!</div>';
        }

        return '<div class="alert alert-danger text-center">Erro ao adicionar a conta para pagamento. Tente novamente.</div>';
    }
    ...
    public function EditarConta($id){
        ...
        $this->db->where('id', $id);
        $update = $this->db->update('contas_pagamento', $dados);

        if($update){

            return '<div class="alert alert-success text-center">Conta para pagamento editada com sucesso!</div>';
        }

        return '<div class="alert alert-danger text-center">Erro ao editar a conta para pagamento. Tente novamente.</div>';
    }
