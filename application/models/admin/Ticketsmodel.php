...
class Ticketsmodel extends CI_Model{
    ...
    public function ResponderTicket($id){
        ...
        if($insere){
            ...
            if($ticketInfo->num_rows() > 0){
                ...
                EnviaNotificacao($row->id_usuario, 'Seu ticket <b>#'.$id.'</b> foi respondido pelo suporte');
        ...
        return '<div class="alert alert-danger text-center">Erro ao responder ticket. Tente novamente.</div>';
    }
    ...
    public function FecharTicket($id){
        ...
        if($ticketInfo->num_rows() > 0){
            ...
            EnviaNotificacao($row->id_usuario, 'Seu ticket <b>#'.$id.'</b> foi marcado como Fechado');
